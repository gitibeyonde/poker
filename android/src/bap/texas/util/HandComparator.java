...
public class HandComparator {
    ...
    public static long[] compareGameHand(long PlayerHand1, long PlayerHand2, 
        ...
        long val1[] = valueOf(PlayerHand1, communityHand, gameType, false);
        long val2[] = valueOf(PlayerHand2, communityHand, gameType, true);
    ...
    public static long[] bestHandOf5(long playerHand, long communityHand, 
                                     int gameType, boolean dealer) {
        long result[] = valueOf(playerHand, communityHand, gameType, dealer);
        long bestHand[] = new long[2];
        bestHand[0] = -99;
        bestHand[1] = -99;
        if (((gameType & HandOps.HOLDEM) == gameType)) {
            bestHand[0] = result[2];
        }
    ...
    public static long[] valueOf(long ph, long ch, int type, boolean dealer) {
        long[] result = new long[4];
        result[0] = -99;
        result[1] = -99;
        result[2] = -99;
        ...
        if (((type & HandOps.HOLDEM) == type) ) {
            long hands[];
            if (dealer){
                hands = getCombinations(ph | ch, 5);
            }
            else {
                hands = getCombinations(ph | ch, 5);
                //remove hands which do not have at least one player card
                
                for (int i=0;i<hands.length;i++){
                    if (!(LongOps.getHighs(hands[i] & ph) < (LongOps.getHighs(hands[i]) + LongOps.getHighs(ph)))){
                       hands[i]=0;
                    }
                }
            }
    ...
    private static synchronized long[] getCombinations(long hand, int nos) {
        assert LongOps.getHighs(hand) <= 9 : "LongOps.getHighs(hand) <= 9" + hand;
        idx = 0;
        int values_size = 1;
