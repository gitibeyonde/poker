...
public class StudHiLo
    extends Stud {
  // set the category for logging
  static Logger _cat = Logger.getLogger(StudHiLo.class.getName());

  public StudHiLo(String name, int minPlayers, int maxPlayers,
                  int rake, double[] maxRake, String[] affiliate,
                  String[] partner,
                  Observer stateObserver) {
    super(name, minPlayers, maxPlayers, rake, maxRake, affiliate, partner,
          stateObserver);
    _type = new PokerGameType(PokerGameType.Play_StudHiLo);
  }
  ...
  public void attachWinners(Pot p) {
    ...
    for (int i = 0; i < v.length - 1; i++) {
      long result[] = HandComparator.compareGameHand(high.getHand().getCards(),
          ...
          communityCards(), PokerGameType.STUDHILO, true);
