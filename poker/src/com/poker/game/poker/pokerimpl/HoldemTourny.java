...
public class HoldemTourny
    extends Holdem implements MTTInterface {

  // set the category for logging
  static Logger _cat = Logger.getLogger(HoldemTourny.class.getName());
  ...
  public Response start() {
    ...
    if (pl.length >= _minPlayers) { // minplayers ==2
      ...
      if (_state != HAND_INIT) {
        ...
        _inquirer = pl[0];
        //_table.remove(pl[0]);  // DO not remove this player from table as he may be waiting for merge
        pl[0].unsetRemoved(); // special case for tourny
      }
      _startCount++;
      _cat.fine("Singe player Tourny state=" + _state);
      ...
      _inProgress = false;
      _startCount++;
      _cat.fine("Cannot start Tourny state=" + _state);
      return new GameDetailsResponse(this);
    }
  }

  public HoldemTourny( String name, int limit, int minPlayers, int maxPlayers,
                      String[] affiliate,
                      Observer stateObserver, Tourny tourny) {
    super(name, minPlayers, maxPlayers, 0, 0, affiliate, null,
          stateObserver);
   _type = new PokerGameType(PokerGameType.HoldemTourny);
    _keepRunning = false;
    _t = tourny;
    _tournyId = _t.name();
    _limit = limit;
  ...
  public void refreshPlayerStatus() {
    ...
    for (int i = 0; i < v.length; i++) {
      ...
      if (v[i].getAmtAtTable() <= 0) {
        ...
        gp.deliver(new ResponseString(1, com.golconda.message.Response.R_PLAYER_REMOVED, name()));
        _cat.finest("Sending player removed to " + v[i]);
        _table.remove(v[i]);
        _observers.add(v[i]);
        removeInvite(v[i]);
    ...
    if (_deb_enlen != _deb_count + _players.length) {
    	_cat.warning("The winner count slipped here");
    }
    resetPartners();
    for (int i = 0; i < _players.length; i++) {
      addInvite(_players[i]);
      _cat.finest("AddInvite " + _players[i]);
    }
  }
  ...
  public void destroyTournyTable() {
    ...
    for (int i = 0; v != null && i < v.length; i++) {
      ...
      gp.deliver(new com.poker.common.message.ResponseString(1,
          ...
          name()));
      _cat.finest("Destroy command send to = " + v[i]);
    }
    for (Iterator<Presence> i = _observers.iterator(); i.hasNext(); ) {
  ...
  @Override
  public Response join(Presence p, double amount) {
    ...
    _inquirer = p;
    setCurrent(p);
    p.joinTable(_t.chips(), new ModuleType(ModuleType.POKER));
    p.unsetBroke();
  ...
  public void attachWinners(Pot p) {
    ...
    Vector<PokerPresence> winner = new Vector<PokerPresence>();
    v[0].setShowdown();
    winner.add(v[0]);
  ...
  public Response moveToTable(Presence p, String tid) {
    ...
    if (!_table.join(p)) { // couldn't be added for whatever reason
      return new GameDetailsResponse(this);
    }
  ...
  public Response gameOverResponse(Presence p) {
    ...
    declarePotWinners();
    _cat.finest("Pot winners declared ");
    new GameOverResponse(this); /// for loggin winner
    _cat.finest("GameOverResponse ");
    ...
    _cat.finest("Pre post run ");
    postRun();
    _cat.finest("post post run ");
    _inProgress = false;
    _state = HAND_OVER;
    Response r = new TournyHandOverResponse(this, showdown_lv);
    _cat.finest("Pre post win ");
    postWin();
    _cat.finest("Post post win ");
    _cat.finest("Tourny state=" + _state);
    return r;
  }
  ...
  public boolean handOver() {
    return _state == HAND_OVER || _state == HAND_NOSTART || _state == HAND_INIT;
  }
  ...
  final static int HAND_OVER = 2;
  final static int HAND_RUNNING = 4;
  final static int HAND_NOSTART = 8;
  int _limit=-1;
