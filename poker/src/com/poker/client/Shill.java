...
public class Shill
  ...
  static Logger _cat = Logger.getLogger(Shill.class.getName());
  ...
  public void startShills() throws Exception {
    ...
    for (pcount--; pcount >= 0; pcount--) {
      ...
      System.out.println("Table count = " + _tc + " curr table=" + k);
      gid = _tm[k]._gname;
      int i = 0;
      boolean expelled = false;
      int tried = 0;
      while (i < _tm[k]._maxPlayer && _tm[k]._playerDetails[i] == -1  ) {
        System.out.println("Player = " + i + " max p=" + _tm[k]._maxPlayer);
        i++;
        tried++;
      ...
      if (!expelled) {
        addPlayer(pn[pcount], pn[pcount], gid, i, _tm[k]._buyin);
      }
      _tm[k]._playerDetails[i] = -1; // mark this as occupied
    }
      ...
      try {
        ...
        while (i.hasNext()) {
          ...
          if (key.isReadable()) {
            ...
            if (r._response_name == Response.R_MOVE && r instanceof ResponseGameEvent) {
  ...
  public class Player {
    ...
    public boolean readBody() throws IOException {
      ...
      r = _channel.read(_b);
      if (_b.hasRemaining()) {
        _cat.finest(_name + " Partial body read " + r);
        if (r == -1) {
          _dead = true;
          _cat.warning(_name + " Marking the client dead as channel is closed ");
        }
        return false;
      }
      _b.flip(); // read complete
      _comstr = new String(_b.array());
      _cat.finest(_last_read_time + ">" + _rseq + " read = " + _comstr);
      resetRead();
      return true;
    }
    ...
    public void join(String tid, int pos, double amt) throws Exception {
      _tid = tid;
      _pos = pos;
      CommandMove ge = new CommandMove(_session, Command.M_SIT_IN, amt, tid);
    ...
    public Response buyChips() throws Exception {
      CommandBuyChips ge;
      ge = new CommandBuyChips(_session, 500, 0);
    ...
    public void processMove(ResponseGameEvent rge) throws Exception {
      ...
      int target_pos = Integer.parseInt(ge.get("target-position")!=null?ge.get("target-position") : "-22");
      if (target_pos !=_pos)return;
      ...
      if (moves != null && moves.length > 0) {
        ...
        else if (moves[0][1].equals("none")) {
          _cat.info("Wait received");
        }
        else {
          _cat.log(Level.WARNING, "Illegal position/move received from the server" + moves[0][1]);
        }
      }
    }
    ...
    public ResponseTableList getTableList() throws Exception {
      CommandTableList ge = new CommandTableList(_session, 0xFFFF);
  ...
  private boolean addPlayer(String name, String password, String gid, int pos, double amt)  {
      try {
  ...
  private void consoleLoop() {
    ...
    while (ok) {
      ...
      public TableMap[] tableMap() throws Exception {
        ...
        _tc = rtl.getGameCount();
        TableMap[] tm = new TableMap[_tc];
        for (int i = 0; i < _tc; i++) {
