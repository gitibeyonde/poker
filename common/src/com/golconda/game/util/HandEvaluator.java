...
//	   DENIS PAPP'S HAND RANK IDENTIFIER CODE:
  ...
  public void setThreadSafe(boolean s) {
    synch = s;
  }
...
// HAND COMPARISON STUFF
   ...
   * the hand is.
   */
  public static int rankHand(Cards h) {
    try {
      if (h == null) {
        return 0;
      }
   ...
   * @return a unique number representing the hand strength of the best
   * 5-card poker hand in the given cards and board. The higher the number,
   * the better the hand is.
   */
  public int rankHand(Card c1, Card c2, Cards h) {
    h.addCard(c1);
    h.addCard(c2);
    int rank;
    if (use_native) {
  ...
  public static int nameHandValueRank(int rank) {
  ...
  public static int nameHandValue(Cards c) {
    return rankHand(c) / ID_GROUP_SIZE;
  }

  public static int nameHandValue(int rank) {
    return rank / ID_GROUP_SIZE;
  }

  /**
   * Given a hand, return a string naming the hand ('Ace High Flush', etc..)
   */
  public static String nameHand(Cards h) {
   ...
   */
  public int compareHands(Cards h1, Cards h2) {
    if (h1 == null && h2 == null) {
      return 0;
    }
    if (h1 == null) {
   ...
   * @return 1 = first hand is best, 2 = second hand is best, 0 = tie
   */
  public int compareHands(int rank1, Cards h2) {
    int r1 = rank1;
    int r2 = rankHand(h2);
   ...
   * the hand is.
   */
//	public synchronized static native int CRankHandFast(int[] cards);
  public static native int CRankHandFast(int[] cards);

//	public static native int CRankHandFast_NonSynch(int[] cards);
   ...
   * the hand is.
   */
//	public synchronized static native int CRankHandFast7(int[] cards);
  public static native int CRankHandFast7(int[] cards);

//	public static native int CRankHandFast7_NonSynch(int[] cards);
...
// CACHED HAND COMPARISON STUFF
   ...
   * may be done later.
   */
  public synchronized int[][] getRanks(Cards board) {
    Cards myhand = new Cards(board);
...
// HAND RANK STUFF
   ...
   */
  public double handRank(Card c1, Card c2, Cards h, int np) {
   ...
   * @return probability of having the best hand.
   */
  public double handRank(Card c1, Card c2, Cards h) {
    int i, j, v;
    Cards myHand = new Cards(h);
    Cards xxHand = new Cards(h);
    myHand.addCard(c1);
    myHand.addCard(c2);
    ...
    return (double) ( (double) (good + (double) (tied / 2)) /
                     (double) (good + bad + tied));
  }
   ...
   * @param dk the deck with all known cards removed
   * @return the hand strength
   */
  public double handRank(Card c1, Card c2, int[][] rankCache, Deck dk) {
    int myRank = rankCache[c1.getIndex()][c2.getIndex()];
    good = bad = tied = 0;
    int j, opRank;

    for (int i = dk.getTopCardIndex(); i < Deck.NUM_CARDS; i++) {
      Card n1 = dk.getCard(i);
      for (j = i + 1; j < Deck.NUM_CARDS; j++) {
        Card n2 = dk.getCard(j);
...
// MORE HAND COMPARISON STUFF (Adapted from C code by Darse Billings)
   ...
   */
  public Cards getBest5CardHand(Cards h) {
    int[] ch = h.getCardArray();
    int[] bh = new int[6];
    int j = Find_Hand(ch, bh);
    Cards nh = new Cards(false);
    for (int i = 0; i < 5; i++) {
      nh.addCard(bh[i + 1]);
    }
    return nh;
  ...
  private void Find_Flush(int[] hand, int[] dist, int[] best) {
    ...
    j = 1;
    while (j <= 5) {
        /* assume all five flushcards will be found before i < 1 */
      if (suitvector[i] >= 1) {
  ...
  private static boolean ID_ExistsStraightFlush(Cards h, Byte straight_high,
                                                byte major_suit) {
    int i;
    int straight;
    byte high;
  ...
  // returns ident value
  private static int ID_KickerValue(byte[] paired, int kickers,
                                    byte[] not_allowed) {
    int i = Card.ACE;
    int value = 0;
    while (kickers != 0) {
      while (i >= 0 &&
             (paired[i] == 0 || i == not_allowed[0] || i == not_allowed[1])) {
        i--;
      }
      if (i < 0) {
        break;
  ...
  private static int ID_KickerValueSuited(Cards h, int kickers, byte suit) {
    int i;
    int value = 0;
   ...
   * the hand is.
   */
  // @TODO: doesn't work with 6 tens: TD TS AH TC TH TS TD
  // but works with 5: TD TS AH TC TH TS 6D
  // I've corrected below size of arrays to POKER_HAND+2 from POKER_HAND+1
  // otherwise exception occured when 6 tens come here
  // now it just incorrectly ranks hand :-)
  public static int rankHand_Java(Cards h) {
    if (h == null) {
      return 0;
    }
    ...
    // pair data
    byte[] group_size = new byte[POKER_HAND + 2];
    byte[] paired = new byte[Card.NUM_RANKS];
    byte[][] pair_rank = new byte[POKER_HAND + 2][2];
    // straight
    ...
    for (r = 0; r < h.size(); r++) {
        //System.out.println("Card=" + h.getCard(r + 1));
      rank = (byte) h.getCard(r + 1).getRank();
      suit = (byte) h.getCard(r + 1).getSuit();
      //System.out.println(r + " >>> " + rank + " " + suit);
      paired[rank]++;
      /*
          System.out.println(group_size.length + " " + paired[rank] + " " + rank + " " + paired.length);
          System.out.print("paired:     ");
          for (int pair_idx = 0; pair_idx < paired.length; pair_idx++)
              System.out.print(paired[pair_idx] + " ");
          System.out.println("");
          System.out.print("group_size: ");
          for (int pair_idx = 0; pair_idx < group_size.length; pair_idx++)
              System.out.print(group_size[pair_idx] + " ");
          System.out.println("");
          System.out.print("suit_size:  ");
          for (int pair_idx = 0; pair_idx < suit_size.length; pair_idx++)
              System.out.print(suit_size[pair_idx] + " ");
          System.out.println("");
       */
    ...
    /*
             System.out.print("pair_rank:  ");
             for (int pair_idx = 0; pair_idx < pair_rank.length; pair_idx++)
     System.out.print(pair_rank[pair_idx][0] + " " + pair_rank[pair_idx][1] + " ");
             System.out.println("");
     */
    // now id type
    int ident;
