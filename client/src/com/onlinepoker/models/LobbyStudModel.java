...
public class LobbyStudModel
   ...
   * Constructor.
   */
  public LobbyStudModel() {
    gameType = 1;//new PokerGameType(POKER_TYPE_HOLDEM);
  }

  public LobbyStudModel(GameEvent ge) {
    super(ge);
    name = ge.getGameName();
...
//    bigBlind = Double.parseDouble(ge.get("big-blind"));
   ...
   * Constructor.
   */
  public LobbyStudModel(LobbyTableModel model,
                          double lowBet,
                          double highBet,
                          double smallBlind,
                          double bigBlind
...
//    setGameLimit(gameLimit);
   ...
   * Constructor.
   */
  public LobbyStudModel(LobbyStudModel model) {
    copy(model);
  }
  ...
  public void copyWithoutSettings(LobbyTableModel model) {
    ...
    LobbyStudModel studModel = (LobbyStudModel) model;
    this.minBet = studModel.minBet;
    this.maxBet = studModel.maxBet;
    this.smallBlind = studModel.smallBlind;
    this.bigBlind = studModel.bigBlind;
    this.playersPerFlop = studModel.playersPerFlop;
    this.tournamentLevel = studModel.tournamentLevel;
  }
  ...
  public boolean equalsByFields(Object obj) {
    if (obj instanceof LobbyStudModel) {
      LobbyStudModel table = (LobbyStudModel) obj;
      return
